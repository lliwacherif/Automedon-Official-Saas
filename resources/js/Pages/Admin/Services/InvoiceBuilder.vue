...
async function fetchSavedInvoices() {
  ...
  try {
    const { data } = await supabase
      ...
      .eq('tenant_id', tenantId)
      .order('created_at', { ascending: false });
    savedInvoices.value = data || [];
  } catch (e) {
    console.error('Error fetching saved invoices:', e);
  } finally {
...
async function saveCurrentInvoice() {
  ...
  try {
    const payload = {
      tenant_id: tenantId,
      invoice_number: previewData.value.invoiceNumber,
      invoice_data: previewData.value,
...
async function deleteSavedInvoice(id: number) {
  if (!confirm('Supprimer cette facture sauvegardée ?')) return;
  try {
    await supabase.from('service_invoices').delete().eq('id', id);
    savedInvoices.value = savedInvoices.value.filter(i => i.id !== id);
    if (viewingSavedId.value === id) {
      previewData.value = null;
...
function getInvoiceTotal(invoice: any): number {
  ...
  return items.reduce((sum: number, i: any) => sum + (i.totalHT || 0), 0);
}

onMounted(() => { fetchAllServices(); fetchB2BClients(); fetchSavedInvoices(); });
</script>
      ...
      <div class="mx-auto flex h-16 max-w-[1500px] items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-2 flex-wrap justify-end">
          ...
          <button 
            ...
            :disabled="savingInvoice || !previewData" 
            class="inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-60"
            :class="invoiceSaved ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-white hover:bg-slate-800'"
          >
            <Check v-if="invoiceSaved" class="h-4 w-4" />
            <Loader2 v-else-if="savingInvoice" class="h-4 w-4 animate-spin" />
            <Save v-else class="h-4 w-4" />
            <span class="hidden sm:inline">{{ invoiceSaved ? 'Sauvegardé !' : 'Sauvegarder' }}</span>
          </button>
    ...
    <main class="mx-auto grid max-w-[1500px] grid-cols-1 gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[320px_minmax(0,1fr)] lg:px-8">
      ...
      <section class="min-h-[70vh] rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <div v-else-if="!previewData" class="h-full">
          <!-- Saved Invoices List -->
          <div v-if="loadingSaved" class="flex h-[60vh] items-center justify-center">
            <Loader2 class="h-8 w-8 animate-spin text-indigo-600" />
          </div>

          <div v-else-if="savedInvoices.length > 0" class="space-y-4">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                <FileText class="w-5 h-5 text-indigo-500" />
                <h3 class="text-base font-bold text-slate-900">Factures sauvegardées</h3>
              </div>
              <span class="text-xs font-bold text-slate-400 bg-slate-50 px-2.5 py-1 rounded-lg ring-1 ring-slate-100">{{ savedInvoices.length }}</span>
            </div>
            <div class="space-y-2.5">
              <div 
                v-for="inv in savedInvoices" 
                :key="inv.id" 
                class="flex items-center gap-4 p-4 rounded-xl ring-1 ring-slate-200 bg-white hover:ring-indigo-200 hover:bg-indigo-50/30 transition-all group"
              >
                <div class="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center shrink-0 ring-1 ring-indigo-100">
                  <FileText class="w-5 h-5 text-indigo-500" />
                </div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <p class="text-sm font-bold text-slate-900 truncate">{{ inv.invoice_number }}</p>
                    <span class="text-[10px] font-semibold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">
                      {{ (inv.service_ids || []).length }} service{{ (inv.service_ids || []).length !== 1 ? 's' : '' }}
                    </span>
                  </div>
                  <div class="flex items-center gap-3 mt-0.5 text-xs text-slate-400">
                    <span>{{ inv.invoice_data?.client?.name || 'Client' }}</span>
                    <span>&middot;</span>
                    <span>{{ formatDate(inv.created_at) }}</span>
                    <span>&middot;</span>
                    <span class="font-semibold text-slate-600">{{ getInvoiceTotal(inv).toFixed(2) }} DT HT</span>
                  </div>
                </div>
                <div class="flex items-center gap-1.5 shrink-0">
                  <button 
                    @click="openSavedInvoice(inv)" 
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-700 bg-indigo-50 rounded-lg ring-1 ring-indigo-200 hover:bg-indigo-100 transition"
                  >
                    <Eye class="w-3.5 h-3.5" />
                    Ouvrir
                  </button>
                  <button 
                    @click="deleteSavedInvoice(inv.id)" 
                    class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-300 hover:text-red-500 hover:bg-red-50 transition opacity-0 group-hover:opacity-100"
                    title="Supprimer"
                  >
            ...
            <div class="pt-4 border-t border-slate-100 flex flex-col items-center text-center">
              <p class="text-xs text-slate-400">Sélectionnez des services pour créer une nouvelle facture</p>
            </div>
          </div>
          ...
          <div v-else class="flex flex-col items-center justify-center h-[60vh] text-center">
            ...
            <h3 class="text-base font-bold text-slate-900 mb-1">Aucune facture</h3>
